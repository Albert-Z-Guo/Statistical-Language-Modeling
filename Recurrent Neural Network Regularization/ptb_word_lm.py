...
flags.DEFINE_string("token_class", "years", "A token class. Possible options are: rounds, days, years.")
...
class PTBModel(object):
    ...
    def __init__(self, is_training, config, input_):
        ...
        self._prob = tf.nn.softmax(logits, axis=-1)
    ...
    def export_ops(self, name):
        ...
        ops[util.with_prefix(self._name, "prob")] = self._prob
    ...
    def import_ops(self):
        ...
        self._prob = tf.get_collection_ref(util.with_prefix(self._name, "prob"))[0]
...
def run_epoch(session, model, eval_op=None, verbose=False):
    ...
    tokens = reader.generate_tokens(FLAGS.data_path, FLAGS.token_class)
    token_positions_generator = reader.generator(model._input.raw_data, model.batch_size, model.num_steps, tokens)
    ...
    for step in range(model.input.epoch_size):
        ...
        positions = next(token_positions_generator)
        if len(positions) > 0:
            prob_arr = []
            for pos in positions:
                prob_arr.append(prob[pos[0]][pos[1]][pos[2]])
            numerator += np.sum(np.log(np.array(prob_arr)))
            num_token += len(prob_arr)
        ...
        if verbose and step % (model.input.epoch_size // 10) == 10:
            if len(positions) > 0:
                num = np.sum(np.log(np.array(prob_arr)))
                den = len(prob_arr)
                print('current token probabilities in batch:', prob_arr)
                print('current token batch perplexity: {:.3}'.format(np.exp(-num/den)))
            print('average token perplexity so far: {:.3}'.format(np.exp(-numerator/num_token)))
            print('number of tokens so far: {}'.format(num_token))

            print("%.3f sequence perplexity: %.3f speed: %.0f wps" %
